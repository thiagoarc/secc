...
try{
	...
	$sql = "SELECT idcontrato, idorgao, CASE WHEN tipo = 1 THEN 'Termo de Adesão' WHEN tipo = 2 THEN 'Licitação' WHEN tipo = 3 THEN 'Compra Direta' end AS tipo, CASE WHEN tipoobjetos = 1 THEN 'Material de Consumo' WHEN tipoobjetos = 2 THEN 'Bens ou Serviços' end AS tipoobjetos, numerotali, DATE_FORMAT(dataassinaturatali, '%d/%m/%Y') as dataassinaturatali, numeroata, DATE_FORMAT(validadeata, '%d/%m/%Y') as validadeata, numeropregao, 
			numeroprocesso, numerocd, numeroparecerjuridico, DATE_FORMAT(datacompra, '%d/%m/%Y') as datacompra, numerocontrato, objeto, valor, DATE_FORMAT(validade, '%d/%m/%Y') as validade, DATE_FORMAT(dataassinatura, '%d/%m/%Y') as dataassinatura, numeroempenho FROM contrato ORDER BY idcontrato desc LIMIT 0, $limit";
